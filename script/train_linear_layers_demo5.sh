...
for dataset in "${datasets[@]}"; do
    ...
    if [ ! -f $model_gen_file ]; then
        python gen_model_answers.py $model_name $dataset $num_samples train 5
    fi
    ...
    calibrator_seq_ckpt="../${dataset}/ICL_cls_ckpt/${model_name_postfix}_calibrator_intenvention_seq_num_demos5.pt"
    if [ ! -f $calibrator_seq_ckpt ]; then
        python train_ICL_seq.py ${model_name} ${dataset} sample 5
    fi
    calibrator_seq_avg_ckpt="../${dataset}/ICL_cls_ckpt/${model_name_postfix}_calibrator_intenvention_seq_avg_num_demos5.pt"
    if [ ! -f $calibrator_seq_avg_ckpt ]; then
        python train_ICL_seq_avg.py ${model_name} ${dataset} sample 5
    fi
    calibrator_seq_last_ckpt="../${dataset}/ICL_cls_ckpt/${model_name_postfix}_calibrator_intenvention_seq_last_num_demos5.pt"
    if [ ! -f $calibrator_seq_avg_ckpt ]; then
        python train_ICL_seq_last.py ${model_name} ${dataset} sample 5
    fi
